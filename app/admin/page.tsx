        ...
        directMembers: [
          {
            id: 200,
            name: "営業部 次長",
            email: "sales.deputy@company.com",
            position: "次長",
            joinDate: "2018-04-01",
            avatar: "/placeholder.svg?height=32&width=32",
          },
        ...
        directMembers: [
          {
            id: 202,
            name: "開発部 アーキテクト",
            email: "dev.architect@company.com",
            position: "シニアアーキテクト",
            joinDate: "2017-03-01",
            avatar: "/placeholder.svg?height=32&width=32",
          },
    ...
    directMembers: [
      {
        id: 102,
        name: "支社長 一郎",
        email: "osaka.manager@company.com",
        position: "支社長",
        joinDate: "2017-04-01",
        avatar: "/placeholder.svg?height=32&width=32",
      },
        ...
        directMembers: [
          {
            id: 203,
            name: "営業部 主任",
            email: "osaka.sales.lead@company.com",
            position: "主任",
            joinDate: "2019-08-01",
            avatar: "/placeholder.svg?height=32&width=32",
          },
...
export default function AdminPage() {
  ...
  const getStatistics = () => {
    ...
    const totalOffices = organizationData.length
    const totalDepartments = organizationData.reduce((total, office) => total + office.departments.length, 0)
    const totalSections = organizationData.reduce(
      (total, office) => total + office.departments.reduce((deptTotal, dept) => deptTotal + dept.sections.length, 0),
      0,
    )
    ...
    return {
      ...
      totalDepartments,
      totalSections,
      monthlyKudos: 1247, // Mock data
      activeUsers: Math.floor(totalMembers * 0.85), // Mock: 85% active
      newMembers: 12, // Mock data for this month
      kudosGrowth: 15.3, // Mock: 15.3% growth
    }
  }
        ...
        <main className="p-4 md:p-8">
          <div className="max-w-7xl mx-auto">
            <div className="mb-6 md:mb-8">
              <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-2">ユーザー管理</h1>
              <p className="text-sm md:text-base text-muted-foreground">組織統計とメンバー管理</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              ...
              <Card className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">今月のKudos</p>
                    <p className="text-2xl font-bold text-foreground">{stats.monthlyKudos}</p>
                    <p className="text-xs text-green-600 flex items-center gap-1">
                      <TrendingUp className="w-3 h-3" />+{stats.kudosGrowth}%
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {Math.round((stats.activeUsers / stats.totalMembers) * 100)}% of total
                    </p>
                  </div>
              ...
              <Card className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">今月の新規登録</p>
                    <p className="text-2xl font-bold text-foreground">{stats.newMembers}</p>
                    <p className="text-xs text-muted-foreground">
                      {stats.totalOffices}事業所 • {stats.totalDepartments}部署
                    </p>
                  </div>
                ...
                <Card key={office.id} className="overflow-hidden">
                  ...
                  <div className="p-4 border-b">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="outline">{office.totalMembers}人</Badge>
                    </div>
                  </div>
                  ...
                  {expandedOffices.includes(office.id) && (
                    <div className="p-4 space-y-4">
                      {office.directMembers && office.directMembers.length > 0 && (
                        <div className="mb-6">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 ml-4">
                            {office.directMembers.map((member) => (
                              <div key={member.id} className="p-3 bg-blue-50 rounded-lg border">
                                <div className="flex items-center gap-3">
                        ...
                        <div key={dept.id} className="border-l-4 border-blue-200 pl-4">
                          ...
                          {expandedDepartments.includes(dept.id) && (
                            <div className="ml-6 space-y-3">
                              {dept.directMembers && dept.directMembers.length > 0 && (
                                <div className="mb-4">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                    {dept.directMembers.map((member) => (
                                      <div key={member.id} className="p-3 bg-green-50 rounded-lg border">
                                        <div className="flex items-center gap-3">
                                          <Avatar className="w-8 h-8">
                                            <AvatarImage src={member.avatar || "/placeholder.svg"} />
                                            <AvatarFallback>{member.name.charAt(0)}</AvatarFallback>
